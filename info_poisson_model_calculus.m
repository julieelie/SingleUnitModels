...
N_mu = length(Mu_in);
P_YgivenS = nan(length(Y_world),N_mu);
P_YgivenS_rescaled = nan(length(Y_world),N_mu);
...
for mm=1:length(Mu_unique)
    ...
    % From logP to P
    P = 2.^(Log_P);
    P_YgivenS(:,(I_mu_unique==mm))=repmat(P',1,sum(I_mu_unique==mm));
   
    % Check that sum(P) sum to 1 and rescale if not to have a more
    % realistic distribution.
    P = P ./sum(P);
    P_YgivenS_rescaled(:,(I_mu_unique==mm))=repmat(P',1,sum(I_mu_unique==mm));
