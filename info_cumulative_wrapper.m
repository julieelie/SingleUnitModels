...
for modelrun=1:mm_local
    ...
    [Icum_EstMonteCarlo(:,modelrun),~]=info_cumulative_model_Calculus(P_YgivenS_allModel{modelrun},'StimIndicesAll',X_stim_indices_wholeset{modelrun},'StimIndicesLast',X_stim_indices_wholeset{modelrun}{mm},'Model#',modelrun,'CalMode','MonteCarlo', 'MCParameter',ParamModel.NumSamples_MC_Cum_Info);
...
if ParamModel.ModelChoice(2) && ~SWITCH.AllAlpha
    ...
    % Semantic Model
    mm_local=mm_local+1;
    Model.Semantic.cum_info.currrent_dataset.ExactHist5(mm,1)=Icum_ExactMem0_5(mm_local);
    Model.Semantic.cum_info.currrent_dataset.EstMonteCarlo(mm,1:2)=Icum_EstMonteCarlo(:,mm_local);
    Model.Semantic.cum_info.currrent_dataset.EstMarkov2(mm,1)=Icum_EstMarkov2(mm_local);
    Model.Semantic.cum_info.currrent_dataset.EstMarkov3(mm,1)=Icum_EstMarkov3(mm_local);
    Model.Semantic.cum_info.currrent_dataset.EstMarkov4(mm,1)=Icum_EstMarkov4(mm_local);
    Model.Semantic.cum_info.currrent_dataset.EstMarkov5(mm,1)=Icum_EstMarkov5(mm_local);
    mm_local=mm_local+1;
